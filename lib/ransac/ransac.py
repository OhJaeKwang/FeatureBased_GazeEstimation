...
# Ellipse RANSAC
def FitEllipse_RANSAC_Support(pnts, roi, max_itts=30, max_refines=10, max_perc_inliers=50.0):
    '''
...
def FitEllipse_RANSAC(pnts, roi, max_itts=30, max_refines=3, max_perc_inliers=50.0):
    '''
    ...
    for itt in range(0,max_itts):
        ...
        # Select 5 points at random
        sample_pnts = np.asarray(random.sample(list(pnts), 8))
        ...
        # Update best ellipse
        best_ellipse = ellipse
        # ellipse_list.append(best_ellipse)

        if perc_inliers > max_perc_inliers:
            if DEBUG: print('Break Max Perc Inliers')
            break

    # # 기준을 만족하는 최고 / 차선의 ellipse 찾기
    # ellipse_list.reverse()
    # for candidate in ellipse_list:
    #     if candidate[2] > 150:
...
def FitCircle_RANSAC(pnts, roi, max_itts=30, max_refines=3, max_perc_inliers=50.0):
    '''
    Robust ellipse fitting to segmented boundary points
    Parameters
    ----
    ...
    for itt in range(0,max_itts):
        ...
        # Select 5 points at random
        sample_pnts = np.asarray(random.sample(list(pnts), 3))

        # Fit ellipse to points
        circle = cv2.minEnclosingCircle(sample_pnts)

        # # Refine inliers iteratively
        # for refine in range(0, max_refines):
        #
        #     # Calculate normalized errors for all points
        #     norm_err = CircleNormError(pnts, circle)
        #
        #     # Identify inliers
        #     inliers = np.nonzero(norm_err**2 < max_norm_err_sq)[0]
        #
        ...
        #     # Protect ellipse fitting from too few points
        #     if inliers.size < 5:
        #         if DEBUG: print('Break < 3 Inliers (During Refine)')
        #         break
        #
        #     # Fit ellipse to refined inlier set
        #     circle = cv2.minEnclosingCircle(inlier_pnts)
        #
        # # End refinement
        #
        # # Count inliers (n x 2)
...
def CircleNormError(pnts, ellipse):
    """
    Error normalization factor, alpha
    Normalizes cost to 1.0 at point 1 pixel out from minor vertex along minor axis
    """
