...
def set_circles_on_img(image, circles_list, circle_size=3, is_copy=False):
    """
    ...
    if isinstance(circles_list[0][0], list) or isinstance(circles_list[0][0], np.ndarray):
        for circle_list in circles_list:
            i = 0
            for circle in circle_list[0:17]:
                cv2.circle(temp_image, (int(np.round(circle[0])), int(np.round(circle[1]))),
                           circle_size, (255, 0, 0), -1)
                # cv2.putText(temp_image, 'B_{}'.format(i), (int(np.round(circle[0])), int(np.round(circle[1]))), \
                #             cv2.FONT_HERSHEY_COMPLEX, 2, (255, 255, 255), 3)
                # i += 1

            for circle in circles_list[17:50]:
                cv2.circle(temp_image, (int(np.round(circle[0])), int(np.round(circle[1]))),
                           circle_size, (0, 0, 255), -1)

            cv2.circle(temp_image, (int(np.round(circles_list[0][0])), int(np.round(circles_list[0][1]))),
                       circle_size, (255, 0, 0), -1)
            cv2.circle(temp_image, (int(np.round(circles_list[17][0])), int(np.round(circles_list[17][1]))),
                       circle_size, (0, 0, 255), -1)

    else:
        for circle in circles_list[1:17]:
            cv2.circle(temp_image, (int(np.round(circle[0])), int(np.round(circle[1]))), circle_size, (0, 0, 255), -1)
        
        for circle in circles_list[18:50]:
            cv2.circle(temp_image, (int(np.round(circle[0])), int(np.round(circle[1]))), circle_size, (255, 0, 0), -1)

        cv2.circle(temp_image, (int(np.round(circles_list[0][0])), int(np.round(circles_list[0][1]))), circle_size, (0, 0, 255), -1)
                   
        cv2.circle(temp_image, (int(np.round(circles_list[17][0])), int(np.round(circles_list[17][1]))), circle_size, (255, 0, 0), -1)
                   
    return temp_image


def show_landmarks(img, lmks, circle_size=1, figsize=(10,8), is_copy=True, save=False, save_path=''):
    """
    ...
    plt.imshow(set_circles_on_img(img, lmks, circle_size=circle_size, is_copy=is_copy))
